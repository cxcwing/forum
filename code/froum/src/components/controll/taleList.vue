<template>
    <div>
        <el-card>
            <div class="searchBox">
                <span style="margin-right: 100px;font-weight: 600;font-size: 18px;">我的故事</span>
...
const handleDelete = (item, index) => {
    ...
    ElMessageBox.confirm(
        ...
        .then(() => {
            axios.delete(`/froumApi/froum/deleteUserTale?id=${item.id}&&authorId=${item.authorId}`).then(res => {
                if (res.data.ok) {
...
onMounted(async () => {
    axios.get(`/froumApi/froum/getUserTale?id=${userForm.value._id}`).then(res => {
        if (res.data.ok) {
