    ...
    <div>
        <el-card v-loading="loading">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <p style="color: black; font-weight:500;">编辑中心</p>
                </el-breadcrumb-item>
                <el-breadcrumb-item>
                    <h2 class="homeXie" href="/">重新编写</h2>
                </el-breadcrumb-item>
            </el-breadcrumb>
            <el-form ref="ruleFormRef" style="max-width: 100%;" :model="taleForm" :rules="rules" label-width="auto"
                ...
                <el-form-item :size="'large'" label="类型" prop="gender">
                    <el-select v-if="!isPost" style="width: 200px;" v-model="taleForm.type" placeholder="选择类型" class="sel">
                    ...
                    <el-select v-else style="width: 200px;" v-model="taleForm.type" placeholder="选择类型" class="sel">
            ...
            <div class="bottom">
                <el-button type="primary" size="large" round @click="handleCommit">更改文章</el-button>
            
                <!-- <el-button type="primary" round></el-button> -->
                <!-- <el-button type="primary" round>发布故事</el-button> -->
            </div>
        </el-card>
        <el-dialog v-model="dialogVisible" title="确认更改" width="500">
            <span>确认要保存更改吗</span>
            <template #footer>
...
const isPost = parseInt(route.params.isPost)
...
const commitSure = async () => {
    ...
    if(isPost){
         res = await upLoad(`/froumApi/froum/postUpdate`, taleForm.value)
    }else{
        res =  await upLoad(`/froumApi/froum/taleUpdate`, taleForm.value)
    }
...
onMounted(()=>{
    ...
    if(isPost){
        ...
        axios.get(`/froumApi/froum/getPost?id=${id}`).then(res =>{
            if(res.data.ok){
        ...
        axios.get(`/froumApi/froum/getTale?id=${id}`).then(res =>{
            if(res.data.ok){
                taleForm.value = res.data.data
            }else{
